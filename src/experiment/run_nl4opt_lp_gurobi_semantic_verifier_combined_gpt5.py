#!/usr/bin/env python3
"""
run_nl4opt_lp_gurobi_semantic_verifier_combined_gpt5.py

Combined pipeline:

  NL4OPT → GPT-5.1 LP JSON (base)
        → GPT-5.1 LP semantic self-check (semantic LP)
        → Gurobi solve

  If Gurobi result is not OPTIMAL or obj is None:
        → GPT-5.1 LP repair using solver feedback (verifier LP)
        → Gurobi solve again

So we have up to 3 GPT calls per example:
  1. Base LP JSON
  2. Semantic self-check LP
  3. Optional solver-feedback LP repair (verifier)

Outputs:
  - CSV: results/nl4opt_gpt5_lp_gurobi_semantic_verifier_full.csv (or *_sample.csv)
  - Summary stats on numeric cases (gt>0 & status=OPTIMAL).
"""
...
class GPT5Caller:
    def __init__(self, model_name: str = "gpt-5.1", max_retries: int = 6):
        load_dotenv()
        self.client = OpenAI()
    ...
    def call(self, prompt: str, temperature: float = 0.0) -> str:
        """
        Minimal wrapper to call GPT with a plain text prompt,
        with exponential backoff on rate-limit errors (429).
        """
        last_error = None
        for attempt in range(self.max_retries):
            try:
                ...
                last_error = e
                sleep_seconds = 2 ** attempt  # 1, 2, 4, 8, 16, 32...
                print(
                    f"[GPT5Caller] Rate limit (429) on model={self.model_name}, "
                ...
                # Transient server-side issues – retry as well
                last_error = e
                sleep_seconds = 2 ** attempt
                print(
                    f"[GPT5Caller] APIError on model={self.model_name}, "
                    f"attempt {attempt+1}/{self.max_retries}. "
                    f"Sleeping {sleep_seconds} seconds..."
                )
                time.sleep(sleep_seconds)

        # If we exhausted all retries, re-raise the last error
        print("[GPT5Caller] Exhausted retries, giving up on this call.")
        if last_error is not None:
            raise last_error
        raise RuntimeError("GPT5Caller: request failed with unknown error")
...
# ==========================================
# 6. Prompts: base LP, semantic self-check, solver-feedback repair
# ==========================================
...
def build_lp_verifier_repair_prompt(
    question: str,
    last_json: str,
    solver_status: str,
) -> str:
    """
    Solver-feedback repair prompt:
    GPT-5.1 sees the story, the last LP JSON, and the solver status (e.g., INFEASIBLE),
    and must repair the LP.
    """
    return (
        "You are an expert in linear programming and mathematical modelling.\n"
        "You formulated the following LP for a word problem, and an external LP solver\n"
        f"reported the status: \"{solver_status}\".\n\n"
        "This indicates that your LP is likely incorrect or incomplete.\n"
        "You must FIX the LP so that it correctly encodes the word problem and is\n"
        "feasible and bounded if the story implies a standard LP.\n\n"
        "Instructions:\n"
        "- Carefully re-read the word problem.\n"
        "- Carefully re-read your previous LP JSON.\n"
        "- Identify why the solver status might be INFEASIBLE, UNBOUNDED, or otherwise wrong.\n"
        "- Regenerate a NEW LP JSON that:\n"
        "  * faithfully encodes every constraint and requirement in the story,\n"
        "  * has the correct objective direction (maximize/minimize),\n"
        "  * includes all resource capacities and percentage/proportion constraints,\n"
        "  * uses integer variables when the story clearly counts items or trips.\n\n"
        "Output format:\n"
        "- Do NOT explain your reasoning.\n"
        "- Output ONLY the final LP as a single valid JSON object\n"
        "  with keys: sense, objective, constraints, var_lower_bounds, var_upper_bounds, integer_vars.\n"
        "- No backticks, no comments, no trailing text.\n\n"
        "Word problem:\n"
        f"{question}\n\n"
        "Previous LP JSON:\n"
        f"{last_json}\n\n"
        "Now output the REPAIRED LP JSON:\n"
    )


# ==========================================
# 7. Combined strategy: base + semantic + solver-feedback repair + Gurobi
# ==========================================

def strategy_lp_gurobi_semantic_verifier(
    caller: GPT5Caller,
    question: str,
) -> Dict[str, Any]:
    """
    Combined LP strategy:
      ...
      4) If solver status is not OPTIMAL or obj_value is None:
            - We call GPT-5.1 again with solver feedback (status + last JSON)
              to repair the LP, then solve again with Gurobi.

    Returns:
      {
        "status": (Gurobi status after final solve),
        "obj_value": float or None,
        "solution": dict(var->val) or None,
        "lp_json_rounds": [base_json, sem_json, maybe verifier_json],
        "num_calls": int,
        "parse_fail": bool,
      }
    """
    ...
    # ---- Step 3: first Gurobi solve ----
    result = solve_lp_with_gurobi(lp)
    status = result["status"]
    obj = result["obj_value"]
    sol = result["solution"]

    # If status is good and obj is valid, we stop.
    ...
    # ---- Step 4: solver-feedback LP repair (verifier) ----
    # Only one extra repair attempt to keep things stable.
    verifier_prompt = build_lp_verifier_repair_prompt(
        question=question,
        last_json=raw_json_sem,
        solver_status=status,
    )
    raw_json_verifier = caller.call(verifier_prompt, temperature=0.0)
    num_calls += 1
    lp_json_rounds.append(raw_json_verifier)

    lp2 = parse_lp_json_to_schema(raw_json_verifier)
    if lp2 is None:
        # If repair JSON is invalid, keep previous status.
        return {
            "status": status,
            "obj_value": obj,
            "solution": sol,
    ...
    result2 = solve_lp_with_gurobi(lp2)
    status2 = result2["status"]
    obj2 = result2["obj_value"]
    sol2 = result2["solution"]
...
def evaluate_nl4opt_lp_gurobi_semantic_verifier(
    n_examples: Optional[int] = None,
    random_state: int = 0,
    tol: float = 1e-3,
):
    """
    Run GPT-5.1 LP+Gurobi with:
      - Semantic self-check
      - Solver-feedback repair (verifier)
    on NL4OPT.
    ...
    for i, row in df_data.iterrows():
        ...
        t0 = time.time()
        result = strategy_lp_gurobi_semantic_verifier(caller, question=q)
    ...
    out_path = (
        "results/nl4opt_gpt5_lp_gurobi_semantic_verifier_full.csv"
        if n_examples is None
        else "results/nl4opt_gpt5_lp_gurobi_semantic_verifier_sample.csv"
    )
...
if __name__ == "__main__":
    ...
    evaluate_nl4opt_lp_gurobi_semantic_verifier(
        n_examples=None,
        random_state=0,
        tol=1e-3,
    )
